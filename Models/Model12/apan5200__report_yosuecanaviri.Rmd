...
The “Predictive Analysis Competition Project” was an exhaustive learning experience where numerous tools and methods learned in class were used to come up with the best model. I personally used seven different predictive methods in R: linear regression(lm),  ANOVA, decision tree(tree), bagging, ranger ,randomForest and boosting(gbm), and support vector machine model (svm).

My first model was a linear regression using tempo and time_signature as the predictive variables. Then I tried using all the numerical variables in a linear regression model to predict rating. Using all the numerical predictors performed well. After this, I tried a few ANOVA and tree models but did not see an improvement until I added hyperparameters. Additionally, I started making data transformations such as eliminating negative ratings from the training set and transforming some numerical variables(such as `key`) to categorical. 

At this point, I realized the importance of data preparation to run models so I started exploring the different variables by visualizing their trends with rating and transforming all the variables that can be categorized to categorical because the numerical variable would cause the predictive models to try to find the linear trend among these categorical variables. After improvement from the data transformation, I kept trying to improve my tree model for a few attempts by using feature selection of variables, but my predictive model score was not improving until I used randomForest model.

The bagging, ranger, boosting and randomForest models (without any tuning) improved my score significantly, but the best score was achieved with randomForest. randomForest was the go-to option for tuning but I was not able to tune my model at this point because it would take too long because of the wide range of training parameters I was using. I tried to manually tune my model by running the model with different parameters different times, but this was not time efficient.

I found myself again not being able to find improvement because I did not know what variables could be helpful if none of the categorical or numerical seemed to need to be categorized based on the trends. But during this time, it was discussed in class to use “performer” words as categorical, so I did that, and I improved my score. 

As I realized I needed to start thinking out of the box to create variables, I also realized I was making the big mistakes:
•	I was testing my model RMSE incorrectly.
o	 I was testing it against my own training data because I was not paying attention to the details of the code but just trying to run multiple models to improve my score
•	I was not working organizing my code properly.
o	I was running multiple models on the same code and had chunks of code that were not relevant.
•	I was not looking at the non-transformed variables (such as genre)
o	I was not thinking about creating variables that were not already part of what was working so far.

When I realized and acknowledged my mistakes, I started fixing these and seeing improvement. First, I started by looking at the character variables (genre, song, performer) and making a few factor variables with the 10 common ones. The new variables improved my model, but I needed to check what type of model to focus on for tuning due to it being time consuming.

Then I fixed my RMSE mistake by simply splitting the “scoring”(training) data set to try the models that were working best at this point (randomForest, boosting and ranger). Again, randomForest seemed to have better performance for this data set, but it was very time consuming in comparison to the others.

Since I was using randomForest, I started looking at the class files and online resources to improve the time efficiency of the model. Using parallel processing allowed me to start tuning with parameters in a long-time frame, but it was done.

As I got to my final models, I started focusing on what worked and improving those to the best of my capabilities. I tuned my randomForest model, started testing the RMSE with the split train/test data, and added new more categorical word (genre, performer, song) variables because they significantly improved the performance of my model previously.

At this point, I was able to test my models before submission, so I did that and compared my tuning results to double check for the best parameters. Then, I noticed big change on the RMSE as I changed the order of the randomForest predictors, so during this part of the tuning process I focused on choosing the most important predictors and only choosing the ones I needed because I had over 100 predictors.

Finally, I submitted my final randomForest model with all the tuned parameters, the most relevant predictors in hierarchical order of importance and improved my score just a little bit. The final leaderboard showed different, and I saw a big improvement in my final model submission score. 

Looking back at the predictive project, I have seen a tremendous improvement in my understanding of different predictive models and the process of training models. Data preparation is equally or more important than the model being used because well prepared and transformed data leads to more efficient performance in the model. Organization is extremely important because small mistakes in the code can make significant differences to what the machine is interpreting. With so many lines of code, variables and functions stored, it is easy to not see what is causing the problem in an unorganized environment. Finally, it is crucial to try different methods and variables and see what works best. In this project, a tuned randomForest model with many categorical variables worked best for me, but I would not have known that if I had not tried many different methods and tools.
